    ...
    for fixture in data.get("response", []):
        if fixture["league"]["id"] in LIGAS_VALIDAS:
            if fixture["fixture"]["status"]["short"] != "NS":
                continue  # Saltar partidos ya empezados
...
def obtener_estadisticas_equipo(equipo_id):
    ...
    prom_goles = round(statistics.mean([g for g in goles if isinstance(g, (int, float))]), 2) if goles else 0
    prom_tiros = round(statistics.mean([t for t in tiros if isinstance(t, (int, float))]), 1) if tiros else "N/A"
    prom_posesion = round(statistics.mean([p for p in posesion if isinstance(p, (int, float))]), 1) if posesion else "N/A"
