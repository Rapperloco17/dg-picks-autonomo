...
WEIGHTS = {"era": 0.4, "runs": 0.4, "form": 0.2}  # Ponderaciones para el puntaje
...
def calcular_puntaje(form, pitcher, cuota):
    """Calcula un puntaje combinado para determinar la seguridad del pick."""
    era_score = max(0, 5 - pitcher["era"]) / 5  # Normalizado, ERA baja = mejor
    runs_score = min(form["anotadas"] / 6, 1)  # Normalizado, máximo 6 carreras
    form_score = max(0, (form["anotadas"] - form["recibidas"]) / 10)  # Diferencia normalizada
    return (WEIGHTS["era"] * era_score + WEIGHTS["runs"] * runs_score + WEIGHTS["form"] * form_score) / cuota

def sugerir_picks() -> list:
    """Genera picks basados en un análisis combinado de estadísticas."""
    cuotas = get_odds()
    juegos = get_today_games()
    picks = []
    processed_games = set()  # Para evitar duplicados
        ...
        for team, form, pitcher in [(home, form_home, p_home), (away, form_away, p_away)]:
            ...
            for threshold in PICK_THRESHOLDS.values():
                if (threshold.get("min_odds", 0) <= cuota <= threshold.get("max_odds", float('inf')) and
                    ...
                    picks.append({
                        "msg": f"{base_msg}\n🎯 Pick: {team} ML @ {cuota} — {threshold['label']} (Puntaje: {puntaje:.2f})",
                        "cuota": cuota,
                        "puntaje": puntaje
                    })
                    break
                ...
                picks.append({
                    "msg": f"{base_msg}\n⚠️ Sin valor de apuesta @ {cuota} (Puntaje: {puntaje:.2f})",
                    "cuota": cuota,
                    "puntaje": puntaje
                })
...
async def main():
    ...
    reto_pick = picks[0] if picks else None  # Pick más seguro
    vip_picks = picks[:3] if len(picks) >= 3 else picks[:len(picks)]  # 2-3 mejores

    # Mensaje para Reto (1 pick más seguro)
