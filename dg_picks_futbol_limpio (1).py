...
# Configurar logging para que los mensajes se envíen a stdout
logging.basicConfig(
    level=logging.INFO,
...
if not API_KEY:
    logging.error("API_FOOTBALL_KEY no está configurada en las variables de entorno.")
...
def obtener_partidos_hoy():
    ...
    try:
        ...
        logging.error(f"Error al obtener partidos de hoy: {e}")
        return []
...
def obtener_cuotas_por_mercado(fixture_id, bet_id):
    try:
        ...
        logging.error(f"Error al obtener cuotas para fixture {fixture_id}, bet {bet_id}: {e}")
        return []
...
def obtener_estadisticas_equipo(equipo_id, condicion):
    ...
    try:
        ...
        logging.info(f"Estadísticas obtenidas para equipo {equipo_id}: {stats}")
        return stats
    except requests.RequestException as e:
        logging.error(f"Error al obtener estadísticas para equipo {equipo_id}: {e}")
...
def calcular_probabilidades_btts_over(equipo_id, condicion):
    try:
        url = f"{BASE_URL}/fixtures?team={equipo_id}&last=20"
        logging.info(f"Calculando probabilidades BTTS/Over para equipo {equipo_id}, condición {condicion}")
        response = requests.get(url, headers=HEADERS)
        response.raise_for_status()
        data = response.json()
        ...
        logging.info(f"Probabilidades calculadas para equipo {equipo_id}: {probs}")
        return probs
    except requests.RequestException as e:
        logging.error(f"Error al calcular probabilidades para equipo {equipo_id}: {e}")
        return {"btts": 0, "over": 0}

if __name__ == "__main__":
    try:
        logging.info("Iniciando script dg_picks_futbol_limpio.py")
        output_file = f"picks_{datetime.now().strftime('%Y%m%d')}.csv"
        logging.info(f"Creando archivo de salida: {output_file}")
        
        # Lista para almacenar picks valiosos (score >= 4)
        picks_valiosos = []
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as f:
            ...
            for p in partidos:
                ...
                if score < 4:
                    logging.info(f"Partido {p['local']} vs {p['visitante']} descartado (score: {score})")
                    continue
        ...
        if picks_valiosos:
            ...
            logging.info("No se encontraron picks valiosos (Score >= 4).")
            print("⚠️ No se encontraron picks valiosos (Score >= 4).")
